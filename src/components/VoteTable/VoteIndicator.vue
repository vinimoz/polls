...
console.log('VoteIndicator props.active =', props.active)

</script>

<template>
	<div class="vote-cell-container">
	<div v-if="pollStore.type === PollType.Generic" class="generic-vote">
	...
	<div v-else :class="['vote-indicator', props.active]" @click="onClick()">
		<MaybeIcon v-if="props.answer === Answer.Maybe" :size="ICON_SIZE" />
		<CheckIcon
			v-if="props.answer === Answer.Yes"
			:fill-color="foregroundColor"
			:size="ICON_SIZE" />
		<CloseIcon
			v-if="props.answer === Answer.No"
			:fill-color="foregroundColor"
			:size="ICON_SIZE" />
	</div>
	</div>
